...
module Entry = struct
  type t =
    ...
    | Shift of T.Labeled_var.t * T.Vertex.t * T.Vertex.t * T.Vertex.t * T.Vertex.t
    ...
    | Predict of T.Vertex.t * T.Vertex.t * T.Var.t list
  [@@deriving eq, ord]

  let pp ppf = function
    | Reduce (x, v, w, u) -> Fmt.pf ppf "@[[REDUCE %a: %a -> ... -> %a <- %a]@]" T.Labeled_var.pp x T.Vertex.pp v T.Vertex.pp w T.Vertex.pp u
    | Shift (x, v, w, v', u) -> Fmt.pf ppf "@[[SHIFT %a: %a -> %a ~> %a <- %a]@]" T.Labeled_var.pp x T.Vertex.pp v T.Vertex.pp w T.Vertex.pp v' T.Vertex.pp u
...
let shift x v w v' u = Entry.Shift (x, v, w, v', u)
...
let predict v w xs = Entry.Predict (v, w, xs)
