...
module Entry = struct
  type t =
    ...
    | Predict of T.Vertex.t * T.Symbol.t list
  [@@deriving eq, ord]
  ...
  let pp ppf = function
    ...
    | Read (x, w, v, u) -> Fmt.pf ppf "[READ '%a': (%a <- %a) <- %a]" T.Symbol.pp x T.Vertex.pp w T.Vertex.pp v T.Vertex.pp u
    ...
    | Predict (v, xs) -> Fmt.pf ppf "[ORDER %a ~ %a ]" T.Vertex.pp v (Fmt.list T.Symbol.pp) xs
end
...
let read x w v u = Entry.Read (x, w, v, u)
