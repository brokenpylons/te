...
module X = Spec.Classic(functor(Context: Spec.CONTEXT) -> struct
  ...
  let parser =
    unextend s u @@ with_ws (T.Vars.of_list lexical) R.(opt (var ws)) Production.[
      ...
      make (progr', program) R.(opt (var ws) * var list_external_declaration);
  ...
  let scanner =
    ...
    let keyword_ = R.(text "auto" + text "double" + text "int" + text "struct" + text "break" + text "else" + text "long" + text "switch" + text "case" + text "enum" + text "register" + text "typedef" + text "char" + text "extern" + text "return" + text "union" + text "continue" + text "for" + text "signed" + text "void" + text "do" + text "if" + text "static" + text "while" + text "default" + text "goto" + text "sizeof" + text "volatile" + text "const" + text "float" + text "short" + text "unsigned") in
    let ident_ = R.(diff (letter * star (letter + digit)) keyword_) in
