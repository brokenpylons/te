...
module X = Spec.Classic(functor(Context: Spec.CONTEXT) -> struct
  ...
  let early_stop =
    let f l0 l1 l2 l3 =
      ...
      | T.Symbol.Code c0, T.Symbol.Code c1, T.Symbol.Code c2, T.Symbol.Code c3 ->
        c0 = code "." &&
        not ((not ((code "A" <= c1 && c1 <= code "Z") || (code "a" <= c1 && c1 <= code "z"))) ||
             ((c1 = code "e" || c1 = code "E") &&
              ((code "0" <= c2 && c2 <= code "9") ||
               ((c2 = code "+" || c2 = code "-") &&
                (code "0" <= c3 && c3 <= code "9")))))
    ...
    in
    [
      (int_literal_constant, f);
      (scalar_int_literal_constant, f);
      (label, f);
    ]
  ...
  let parser =
    unextend s u @@ with_ws (T.Vars.of_list lexical) R.(opt (var ws)) @@ Production.[
      make (u, start) R.(var executable_program * var e * var e');
      make (u, e') R.(var e * var e' + null);

      make (u, executable_program) R.(opt (var ws) * plus (var program_unit));
...
let _ =
  ...
  X.Run.file (fun c -> d#read c) "linear/sample6006.fortran";
